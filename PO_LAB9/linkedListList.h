...
public:
	...
	bool seqSearchIter(elemType block);
	bool seqSearchRec(elemType block);
...
template<class elemType>
bool LinkedListList<elemType>::seqSearchIter(elemType block){
	Node<elemType>* current=myListHead;
	while(current!=NULL){
		if(current->element==block){
			return true;
		}
...
template<class elemType>
bool LinkedListList<elemType>::seqSearchRec(elemType block){
	if(myListHead==NULL){
		return false;
	}
	if(myListHead->element==block){
		return true;
	}
