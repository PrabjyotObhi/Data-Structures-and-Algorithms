...
queueType<elemType>::queueType(int queueSize){
	if(queueSize<=0){
		cout<<"The size of the array to hold the queue must be positive."<<endl;
		cout<<"Creating the an array of size 100. "<<endl;
		maxQueueSize=100;
	}
	else{
	...
	list= new elemType[maxQueueSize];
	size=0;
}

template<class elemType>
queueType<elemType>::~queueType(){
	delete []list;
}

template<class elemType>
bool queueType<elemType>::isEmpty(){
	return(size==0);
}

template<class elemType>
bool queueType<elemType>::isFull(){
	return(maxQueueSize==size);
}

template<class elemType>
elemType queueType<elemType>::front(){
	if(!isEmpty()){
...
template<class elemType>
elemType queueType<elemType>::back(){
	if(!isEmpty()){
...
template<class elemType>
void queueType<elemType>::addQueue(const elemType &block){
	if(!isFull()){
		queueBack=(1+queueBack)%maxQueueSize;
		list[queueBack]=block;
		size++;
	}
	else{
		cout<<"Cannot insert a value as the queue is full-Terminating program."<<endl;
		exit(0);
	}
}

template<class elemType>
void queueType<elemType>::deleteQueue(){
	if(!isEmpty()){
		queueFront=(queueFront+1)%maxQueueSize;
		size--;
	}
	else{
		cout<<"Cannot dequeue because the queue is empty-Terminating program"<<endl;
		exit(0);
	}
}
